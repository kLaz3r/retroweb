...
export async function generateStaticParams() {
  try {
    ...
    const gbaGames = await db
      .select({ id: games.id })
      .from(games)
      .where(eq(games.platform, "gba"));

    return gbaGames.map((game) => ({
      game: game.id.toString(),
    }));
  } catch (error) {
    console.error("Error generating static params for GBA games:", error);
    return [];
  }
}
...
export async function generateMetadata({
  ...
  try {
    ...
    if (!gameRecord?.name || gameRecord.platform?.toLowerCase() !== "gba") {
      return {
        title: "Game Not Found | Retroweb",
      };
    }
    ...
    return {
      title: `${gameRecord.name} - Play Online GBA Game | RetroWeb`,
      description: `Play ${gameRecord.name} online in your browser! Experience this classic GBA game with RetroWeb's emulator. No downloads required - start playing instantly.`,
      keywords: [
      ...
      alternates: {
        canonical: `/emulator/gba/${gameId}`,
      },
      openGraph: {
        title: `${gameRecord.name} - Play Online | RetroWeb`,
        description: `Play ${gameRecord.name} online in your browser! Experience this classic GBA game with RetroWeb's emulator.`,
        type: "website",
        images: [
          {
            url: "/images/gba.svg",
...
const EmulatorPage = async ({ params }: GamePageProps) => {
  try {
    ...
    if (gameRecord.platform.toLowerCase() !== "gba") {
      notFound();
    }
